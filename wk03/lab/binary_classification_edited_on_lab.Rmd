...
## for each row the sum is 1
## YES = the probability of not showing up
```
```{r}
...
## this is better now because I finally have true positives and false negatives are lower now, but accuracy is lower too
```
...
## 55% means the probability of labeling something as positive is 55%
```
...
train_control <- trainControl(method = "cv",
                              number = 5,
                              classProbs = TRUE, # it is necessary
                              verboseIter = TRUE, # i can leave it out ... it controls what will be printed about teh iteration
...
## (in caret ROC is the same as AUC. AUC is the measure  of accuracy here.)
## we can instruct caret to focus on area under the curve instead of accuracy
## every ROC is the mean of 5 ROCs, because we are doing a 5-fold cross validation
glmnet_model
# in every line for each of the hyperparameters we have the average of the 10 different AUCs (if it was a 10-fold CV)
```
...
ggplot(roc_df) +
  ...
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, .1)) +
  xlab("False Positive Rate") + ylab("True Positive Rate") 
## the predictive power is much higher, it is about 72% in the test set
## for any false positive rates I can achive a better true positive  rate with this blue model
## the blue is clearly better, i have a better true positive rate for the same false positive rate
## there is no model chosen here, there are only scores
## depending from the exercise / application i have to choose a treshold from the model, which I am gonna use
```
