...
In task 2.1 we have used `NbClust` heuristics to find the best number of clusters. Two was chosen by the model using the majority rule. All the results of `NbClust` have been loaded to list named `nb`, and it has an attribute labelling all observations into two cluster. In task 2.3 we have computed the first two principal components of all observation using PCA. Now I only had to pair the cluster labelling with the observations and their first two principal components, please find the plot below:
...
There seems to be a (perhaps linear and quadratic) relationship between the two variables.
