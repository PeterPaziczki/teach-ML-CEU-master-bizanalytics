...
## 3. Fashion image classification using `keras`
...
First I am going to build a benchmark and will try to improve it. 
...
### 3.2.2 Trying to improve the model
...
(Note: I experimented with different architectures, with 128 and 512 units on the first layer, but both gave significantly worse results than with 256. I also experimented with different dropout rates, 0.55 seemed to be the optimal choice. ``0.8`, `0.4` and `0.6` gave worse results.)
...
(Note: I experimented with different loss functions, such as `mean_squared_error`, `categorical_crossentropy`, `kullback_leibler_divergence`, `cosine_proximity`, `mean_squared_logarithmic_error`, `categorical_hinge`, `squared_hinge`, `poisson` and `logcosh`, but couldn't make test performance better than 90%. I tried other optimizers, but found `optimizer_adamax()` the best one. `epochs` was set to 100 in early trials, but I found 75 more than enough in all cases.)
...
I tried to improve the model, but couldn't achieve significant improvement compared to the first model with less layers and `epochs`.
...
This `layer_conv_2d` layer creates a convolution kernel that is convolved with the layer input to produce a tensor of outputs.

```{r}
cnn_model <- keras_model_sequential() 
cnn_model %>% 
  layer_conv_2d(filters = 32,
                kernel_size = c(3, 3), # An integer or list of 2 integers, specifying the width and height of the 2D convolution window
                activation = 'relu',
...
Test set performance could be significantly improved!