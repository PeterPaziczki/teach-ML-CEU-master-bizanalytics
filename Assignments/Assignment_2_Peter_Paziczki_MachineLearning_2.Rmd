...
more_epochs_model <- h2o.deeplearning(x = X, 
                 ...
                 hidden = c(32, 32, 32, 32, 32),
                 epochs = 200,
                 stopping_rounds = 0, # setting it to zero means there is no stopping early stopping applied
                 seed = 123)
```
...
input_dropout_model <- h2o.deeplearning(x = X, 
                 ...
                 validation_frame = data_valid,
                 hidden = c(32, 32, 32, 32, 32),
                 input_dropout_ratio = 0.4, # the above showed dropout thign can be applied to the input layer as well
                 seed = 123)
```
...
regularized_model <- h2o.deeplearning(x = X, 
                 ...
                 validation_frame = data_valid,
                 hidden = c(32, 32, 32, 32, 32),
                 # NN is a highly parameterized model, every weight is a parameter ...
                 l1 = 0.001,
                 l2 = 0.001,
                 seed = 123)
```
...
The best performing model was the `benchmark_gbm` model on the validation set with AUC of `0.7201964`.
