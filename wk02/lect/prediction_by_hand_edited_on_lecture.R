...
RootMeanSquaredError <- function(real_value, predicted_value) {
  return(sqrt(mean((real_value - predicted_value) ^ 2))) # mean squared error
}
...
ModelByQuarterEverything <- function(train, test) {
  new_test = copy(test)
  for (q in unique(train$quarter)) {
    for (w in unique(train$workingday)) {
      for (a in unique(train$weather)) {
        for (t in unique(train$temp)) {
          for (h in unique(train$holiday)) {
            for (b in unique(train$hour)) {
      new_test[quarter == q & workingday==w & weather == a & temp == t & holiday == h & hour == b, 
               prediction := mean(train[quarter == q & workingday == w & weather == a & temp == t & holiday == h & hour == b]$count) ]
    }
      }
    }
      }
...
RootMeanSquaredError(bikeTrain$count, ModelByQuarterEverything(bikeTrain, bikeTrain))
