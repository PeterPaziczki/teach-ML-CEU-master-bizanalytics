...
fit_control <- trainControl(method = "repeatedcv", 
                            ...
                            selectionFunction = "oneSE") # one standard error - I am gonna choose the simplest model, whom performance is
...
set.seed(857) # to be able to compare models I need to set the seed
## it is not enough to set it only once, I need to set it every time just before the train function, because the random number
## generator changes all the time. If it is set to the same seed, the data will be cut into the same chunks
rpart_fit <- train(log_price ~ . -price, 
                   data = data_train, 
                   method = "rpart", 
                   trControl = fit_control,
...
lasso_fit <- train(log_price ~ . -price, 
                   data = data_train, 
                   method = "glmnet", # if I use glmnet, I might want to use preprocess
                   preProcess = c("center", "scale"), # some models perform better, if the data is standardized before running training
...
resamps <- resamples(list("regression tree" = rpart_fit,
                          "lasso" = lasso_fit,
                          "linear" = linear_fit))
summary(resamps) # comparing models, lasso is the best in this case. lasso has smaller errors.
```
```{r}
diff_resamples <- diff(resamps)
dotplot(diff_resamples) # it is like a significance test, it takes pairs of modesl and tells that the difference between models is
...
```{r}
# taking the best model and evaluating it
